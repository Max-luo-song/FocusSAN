...
from scipy.spatial import KDTree as kdtree_plus
...
color_tree = kdtree_plus(Scanetv2_40_colors_rgb)
...
def main(
    ...
    if label_space == 'wordnet':
        ...
        for item in get_occ11():
            color_map[item['id']] = item['color']
    elif label_space == 'color_habitat':
        num_classes = len(Scanetv2_40_colors_rgb)
    ...
    '''
        投票机制增加的目的：在voxel_down_sample的过程中，体素内包含多个颜色点，体素的颜色默认是随机选择
        增加一个投票机制可以按照体素中颜色最多的点决定体素的颜色
    '''
    ### voxel_down_sample_and_trace表示下采样并且追踪索引，inverse_index_list代表下采样点在原始点云的索引
    '''
        inverse_index_list = [
            (0, [1, 2, 3]),    # 下采样点 0 对应原始点 [1, 2, 3]
            (1, [4, 5]),       # 下采样点 1 对应原始点 [4, 5]
            (2, [6, 7, 8, 9])  # 下采样点 2 对应原始点 [6, 7, 8, 9]
        ]
    '''
    ...
    ### pc_voxel_down.ply是下采样点云，没颜色的
    o3d.io.write_point_cloud(str(scene_dir / 'pc_voxel_down.ply'), voxel_down_pcd)

    ### 投票机制：每个下采样点根据原始点的投票结果，选择投票得票最多作为语义标签
    ### 将原始点云的语义标签映射到下采样后的点云
    voxel_votes = np.zeros((len(voxel_down_pcd.points), num_classes), dtype=int)
    all_indices = np.concatenate(inverse_index_list)
    ### all_sem_labels 是一个一维数组，形状与 all_indices 相同，记录每个原始点对应的下采样点的索引
    '''
        all_sem_labels = np.concatenate([
        np.full(3, 0),  # [0, 0, 0]
        np.full(2, 1),  # [1, 1]
        np.full(4, 2)   # [2, 2, 2, 2]
    ])
    '''
    ### all_sem_labels:[0, 0, 0, 1, 1, 2, 2, 2, 2]
    all_sem_labels = np.concatenate([np.full(len(indices), i) for i, indices in enumerate(inverse_index_list)])
    ### label_3d 是原始点云的语义标签数组。
    sem_labels = label_3d[all_indices].astype(int)
    ### all_sem_labels表示原始点属于哪个下采样点，sem_labels表示原始点云的语义标签数据
    ### 相当于在voxel_votes中，all_sem_labels记录每个voxel中有哪些点，对这些点的语义标签进行统计
    np.add.at(voxel_votes, (all_sem_labels, sem_labels), 1)  ### voxel_votes相当于下采样数，all_sem_labels相当于每个下采样voxel中包括的点，sem_labels相当于原始点语义编号，
    ...
    ### pc_voxel_down_color.ply是带颜色的下采样，颜色是投票机制
    ...
    scene_voxels = points  # (下采样N，3)
    scene_voxels_sem = voxel_sem[valid_point_mask]  # 每个scene_voxels的语义标签(在color_map的编号)
    ##############################
    ...
    for idx, file in tqdm(enumerate(files), total=len(files)):
        ...
        cam2world = pose  ### colmap的Pose是cam2world，其余应该是world2cam

        voxOriginWorld = cam2world[:3, :3] @ voxOriginCam + cam2world[:3, -1:]  ### 相机旋转+平移=相机原点的世界坐标系
        print("voxOriginWorld Origin:", voxOriginWorld)
        ...
        voxOriginWorld -= delta   ### 这个偏移应该是默认的情况，适用于0.08的点云Occ(可能是occ-scannet标准)
        print("voxOriginWorld - delta:", voxOriginWorld)
        print("scene_voxels[:, 0] max:", scene_voxels[:, 0].max())
        print("scene_voxels[:, 1] max:", scene_voxels[:, 1].max())
        print("scene_voxels[:, 2] max:", scene_voxels[:, 2].max())
        print("scene_voxels[:, 0] min:", scene_voxels[:, 0].min())
        print("scene_voxels[:, 1] min:", scene_voxels[:, 1].min())
        print("scene_voxels[:, 2] min:", scene_voxels[:, 2].min())
        # 保留距离原点4.8范围内的场景  ### voxOriginWorld.reshape(-1)多维数组转换成一维数组
        ...
        print("frame_voxels before:", scene_voxels.shape)
        # ================> debug
        if vis_debug:   ### 保存每一帧的ply文件，从全局ply中过滤局部ply
        ...
        print("voxOriginWorld[0, 0]:", voxOriginWorld[0, 0])
        print("voxOriginWorld[1, 0]:", voxOriginWorld[1, 0])
        print("voxOriginWorld[2, 0]:", voxOriginWorld[2, 0])
        ...
        if vis_debug:
            ...
            ### 把当前帧放在有效全局下的点云保存，有效全局是黑色，当前帧有自己的颜色
        ...
        print("frame_voxels shape  111:", frame_voxels.shape)
        ...
        print("frame_voxels shape:", frame_voxels.shape)
        # rgb_labels = frame_voxels[..., :3]  # 提取前三个通道 (480, 640, 3)
        # print("rgb_labels shape:", rgb_labels.shape)
        # frame_voxels = rgb_to_label(rgb_labels)  # 转换为 (480, 640)
        # print("after frame_voxels shape:", frame_voxels.shape)
        ### b把frame_voxels变成(129600, 4) 前3维是位置，后一维是color索引

        # if vis_debug:
        #     # 仅保存有效的语义
        #     valid_frame_voxels = frame_voxels[np.logical_and(frame_voxels[:, -1] > 0, frame_voxels[:, -1] < 41)]
        ...
        #     # 保留empty区域+语义区域
        #     valid_frame_voxels = frame_voxels[frame_voxels[:, -1] < 41]
        ...
        if vis_debug:    ### 相机可以看到的
            # 可视化过滤后的结果
            visible_voxels = np.hstack((gridPtsWorld, final_labels.reshape(-1, 1)))
            visible_voxels = visible_voxels[np.logical_and(visible_voxels[:, -1] > 0, visible_voxels[:, -1] < 256)]
            print("visible_voxels shape:", visible_voxels.shape)
        ...
        # <<<  新的最终可见性过滤 >>>
        # ===================================================================
        print("frame_voxels gls shape:", frame_voxels.shape)
        ###### 保存
        target_1_4 = frame_voxels[:, -1].reshape(60, 60, 36)  ### 提取最后一列
        print("target_1_4 shape:", target_1_4.shape)
        pkl_data = {
...
def arg_parser():
    ...
    parser.add_argument(
        '--label_space',
        default='color_habitat'
    )  # ['wordnet','occ11', 'color_habitat']
